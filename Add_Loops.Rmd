...
Rand<-function (Population){
  Sp_select<-selectInd(Population,nInd=100,trait=1,use="rand",simParam=SP)
  return(list=Sp_select)
}

Tops<- function(Population){
  Sp_select<-selectInd(Population,nInd=100,trait=1,selectTop=TRUE,use="pheno",simParam=SP)	
 	return(llist=Sp_select)
}
...
### This cannot be start as new chunk?   
```{r include=TRUE}  
