    ...
    for (nDH in c(25)){
      for (varE in c(1.22, 2)){
        for (Ne in c(60, 600)){
          ...
          allRes <- mclapply(1:2, function(dummy) {print(dummy); runOneRep(selection, nPheno, nDH, varE, Ne)}, mc.cores=20)
          
          Mean_g_Rep<-matrix(nrow=cycles,ncol=nrep)
          Sd_g_Rep<-matrix(nrow=cycles,ncol=nrep)
          
          for (i in 1:100){
            Mean_g_Rep[,i] <- allRes[[i]]$mean_g1
            Sd_g_Rep[,i] <- allRes[[i]]$sd_g1
          }
